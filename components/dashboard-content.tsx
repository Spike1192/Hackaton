...
const conflicts = [
  {
    id: 1,
    type: "classroom",
    message: "Salón 302 tiene conflicto el Lunes 10:00-11:00",
  },
  {
    id: 2,
    type: "teacher",
    message: "Prof. García excede 4h diarias el Martes",
  },
];

export function DashboardContent() {
  const totalTeachers = teachersData.length;
  const completeTeachers = teachersData.filter(
    (t) => t.status === "complete"
  ).length;
  const totalCourses = coursesData.length;
  const completeCourses = coursesData.filter(
    (c) => c.hoursAssigned >= c.hoursRequired
  ).length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {completeTeachers} con carga completa
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {completeCourses} con horario completo
            </p>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">
              Progreso de horas asignadas por profesor
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            {teachersData.map((teacher) => {
              const percentage =
                (teacher.hoursAssigned / teacher.hoursRequired) * 100;
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">
              Horas asignadas vs requeridas por curso
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            {coursesData.map((course) => {
              const percentage =
                (course.hoursAssigned / course.hoursRequired) * 100;
